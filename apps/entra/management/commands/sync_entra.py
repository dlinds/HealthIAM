...
class Command(BaseCommand):
    help = (
        "Mirror Entra ID groups over Microsoft Graph. Intended for a scheduled job (cron, or a "
        "Windows Scheduled Task): manage.py sync_entra [--dry-run]. Every run is recorded under "
        "Admin > Entra ID; the exit code is non-zero when the run failed or any row had an error."
    )
    ...
    def handle(self, *args, **options):
        ...
        run = EntraSyncRun.objects.create(trigger=EntraSyncRun.Trigger.SCHEDULED)
